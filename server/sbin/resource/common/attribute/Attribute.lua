...
function DefineAttribute(AttrDefine,Exprs,InfRef,SyncRef,PropRef)
	...
	function Attribute:setValue(value)
		...
		if propID then
			...
			if not peer then
				print("实体没有指定同步集合",toString(owner))
				print("实体类型，dbid",owner:getEntityType(),owner:getDBID())
				print("实体同步属性没有指定属性集合，属性名称为",self:getName())
			end
			setPropValue(peer,propID,value)
		end
	...
	-- 加值
	function Attribute:addValue(value)
		value = tonumber(value)
		if self:isExpr() then
			notice("公式属性 %s 不能加值",self:getName())
			return
		end
	...
	function Attribute:notifyInfluenceChanged()
		...
		for _,attrName in ipairs(inf) do
			...
			if SyncRef[attrName] then
				...
				attribute:notifyInfluenceDirty()
			end

			if not attribute:isExpr() then
				notice("配置错误,%s 并不是一个公式属性",attribute:getName())
			end
		end
	end

	function Attribute:notifyInfluenceDirty()
		local inf = InfRef[self.type]
		local owner = self.entity
		if inf then
			for _,attrName in ipairs(inf) do
