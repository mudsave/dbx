...
function RemoteEventProxy.send(event, player)
	...
	if not player:getGateID() or not player:getClientLink() then
		print ("[Warning!]RPC Target player not exists!")
		return
	end
	RPCEngine:sendToPeer(player:getGateID(), player:getClientLink(), eventID, g_serverId, unpack(params,1,params.n))
end
