...
function PlayerAttrbuteFormula.player_str(player)
	...
	--print("武力：player_in_str,player_str_point,player_add_str,player_add_base_attr",player_in_str,player_str_point,player_add_str,player_add_base_attr)
...
function PlayerAttrbuteFormula.player_sta(player)
	...
	local player_add_base_attr = player:getAttrValue(player_add_base_attr)
	--print("根骨：player_in_sta,player_sta_point,player_add_sta,player_add_base_attr",player_in_sta,player_sta_point,player_add_sta,player_add_base_attr)
...
function PlayerAttrbuteFormula.player_win_phase(player)
	...
	--print("风相性：player_win_phase_point,player_add_win_phase,player_add_all_phase",player_win_phase_point,player_add_win_phase,player_add_all_phase)
...
function PlayerAttrbuteFormula.player_ice_phase(player)
	...
	local player_add_all_phase = player:getAttrValue(player_add_all_phase)
	--print("冰相性：player_ice_phase_point,player_add_ice_phase,player_add_all_phase",player_ice_phase_point,player_add_ice_phase,player_add_all_phase)
...
--生命上限 =（根骨*20+冰相性*30+等级^2/5-等级+生命上限加值 + 红蓝上限加值）*（1+生命值上限加成 + 红蓝上线加成）
...
--法力上限 =（武力*10+智力*10+冰相性*30+等级^2/5-等级+ 法力上限加值 + 红蓝上限加值）*（1+法力值上限加成+红蓝上线加成）
...
--物理攻击力 =（武力*10+火相性*7+等级^2/10-等级+物理攻击力加值+全部攻击力加值）*（1+物理攻击力加成+全部攻击力加成）
...
--法术攻击力 =（智力*10+雷相性*7+等级^2/10-等级+法术攻击力加值+全部攻击力加值）*（1+法术攻击力加成+全部攻击力加成）
...
--物理防御力 =（根骨*5+土相性*4+等级^2/10-等级+物理防御力加值+全部防御力加值）*（1+物理防御力加成+全部防御力加成）
...
--法术防御力 =（根骨*5+土相性*4+等级^2/10-等级+法术防御力加值+全部防御力加值）*（1+法术防御力加成+全部防御力加成）
...
--命中 =（武力*0.5+智力*0.5+敏锐*1+等级*0.5+命中加值）*（1+命中加成）
...
--暴击 =（敏锐*2+风相性*3+等级/2+暴击加值）*（1+暴击加成）
