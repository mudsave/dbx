...
function TaskDoer:doRecetiveTask(player, taskID, GM)
	...
	-- 再判断任务条件是否满足
	if NormalTaskDB[taskID]	then
		if (not	TaskCondition.normalTask(player, taskID, true, GM))	and	(not TaskCondition.guideTask(player, taskID, true, GM) )then
			print("任务条件不满足")
			return false, 2
		end	
...
function TaskDoer:loadNormalTask(player, recordList)
	if recordList then
		...
		if table.size(recordList) == 0 then
			local hisTask = taskHandler:getHisTasks()
			if not hisTask then
				--开场对话
				...
				-- 防止玩家没有任务，
				local hisTaskID	= hisTask[1]
				local hisTaskConfig	= NormalTaskDB[hisTaskID]
				if hisTaskConfig then
					local taskID = hisTaskConfig.nextTaskID
					if taskID then
	...
	if recordList then
		for	_, taskData	in pairs(recordList) do
			if taskData.historyTasks then
				local exec,errMsg =	loadstring("return "..taskData.historyTasks)
				if exec	then
					local c	= exec(	)
					taskData.historyTasks =	c
					player:getHandler(HandlerDef_Task):loadHistoryTask(taskData.historyTasks)
				else
					--print("LoadString	Error",taskData.historyTasks)
					print("loadHistoryTask(player, recordList)error! : playerDBID= ", player:getDBID())
					return
				end
			end
		end
...
-- 加载已完成的指引任务的列表{{taskID = 15015}, {taskID = 15016}, {taskID = 15017}}
function TaskDoer:loadGuideHisTask(player,recordList)
	local guideHisTasks = {}
	if recordList then
		for _,guideTask in pairs(recordList) do
			local taskID = guideTask.taskID
			table.insert(guideHisTasks,taskID)
		end
	end
	if #(guideHisTasks) then
		player:getHandler(HandlerDef_Task):setGuideHisTasks(guideHisTasks)
	end
end
	...
	for	taskID,taskData	in pairs(NormalTaskDB) do
		if taskData.taskType2 == TaskType2.NewBie then
			if taskData.level[1] <=	player:getLevel() then	-- 指引任务不能重复接
				if not player:getHandler(HandlerDef_Task):isGuideHisTask(taskID) then
