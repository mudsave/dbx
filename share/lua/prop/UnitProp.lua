...
DefineProperty(UNIT_NAME,		"STRING",	"noname",	1)	--实体名称
DefineProperty(UNIT_MODEL,		"SHORT",	11,			1)	--实体模型
DefineProperty(UNIT_SHOWPARTS,	"STRING",	"{1,1}",	1)	--显示细节
...
function InitPropSet(cls,propConfig)
	...
	for _,conf in ipairs(Properties) do	--添加lua中共有属性
		local propId,type,default,public = unpack(conf)
		local id = unitConfig:addProperty(cls,type,default,public)
		if propId ~= id then
	...
	for _,conf in ipairs(propConfig) do
		local propName,type,default,public = unpack(conf)
		local propId = unitConfig:addProperty(cls,type,default,public)
		_G[propName] = propId
	end
end
